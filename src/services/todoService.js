...
const addTask = async (taskData) => {
  ...
  try {
    ...
    throw error.response?.data || 'Failed to add task';
  }
};
...
// ✅ New function for checkbox status update
const updateTaskStatus = async (taskId, completed) => {
  const userId = getUserId();
  try {
    const response = await api.put(`${TODO_API_URL}/updatestatus/${userId}/${taskId}`, { completed });
    return response.data;
  } catch (error) {
    throw error.response?.data || 'Failed to update task status';
  }
};
