...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await loginUser(formData); // This will store userId & username in localStorage
      navigate('/tasks');
    } catch (error) {
      ...
      <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '10px', width: '350px', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: '15px' }}>
            ...
            <input type="text" name="username" value={formData.username} onChange={handleChange} style={{ width: '100%', padding: '10px', border: `1px solid ${errors.username ? 'red' : '#ccc'}`, borderRadius: '5px' }} />
            {errors.username && <span style={{ color: 'red', fontSize: '0.85rem' }}>{errors.username}</span>}
          </div>
          
          <div style={{ marginBottom: '15px' }}>
            <label htmlFor="password" style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Password</label>
            <input type="password" name="password" value={formData.password} onChange={handleChange} style={{ width: '100%', padding: '10px', border: `1px solid ${errors.password ? 'red' : '#ccc'}`, borderRadius: '5px' }} />
            {errors.password && <span style={{ color: 'red', fontSize: '0.85rem' }}>{errors.password}</span>}
          </div>
          
          <button type="submit" style={{ width: '100%', padding: '10px', backgroundColor: '#007bff', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer' }} disabled={isLoading}>
